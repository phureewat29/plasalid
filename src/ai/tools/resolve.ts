...
const DEFS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Create a recurrences row and link every supplied transaction to it. Computes first_seen_date, last_seen_date, and next_expected_date from the member transactions. Use this after the user confirms a recurrence_candidate unknown.",
    input_schema: {
      type: "object",
      properties: {
        account_id: {
  ...
  },
  {
    name: "merge_accounts",
    description:
      "Move every posting on `from_id` over to `to_id`, then delete the source account. Use to apply a similar_accounts unknown's 'Merge A into B' resolution. Refuses if the source still has child accounts.",
    input_schema: {
      type: "object",
