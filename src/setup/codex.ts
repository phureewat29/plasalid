...
const CODEX_END_MARKER = "<!-- END plasalid-skill -->";
...
**Report:** net worth via \`plasalid status --json\` (\`net_worth\` block); \`report --from --to\`, \`accounts tree\`, \`transactions list\`, \`transactions dedupe\`. Same payment on two statements: find the pair with \`transactions dedupe\`/\`list --amount\`, then \`transactions merge --from <tx:id> --to <tx:id> --yes\` to void the mirror into the survivor.

**Privacy:** read output is PII-redacted by default; placeholders like \`[USER]\`/\`[CARD]\` are mask placeholders, not literal data — never write them back into the ledger; pass \`--no-redact\` only when the human asks for verbatim text.
